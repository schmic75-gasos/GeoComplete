...
export async function POST(req: Request) {
  try {
    ...
    await sql`
      INSERT INTO user_solved_quests (osm_user_id, quest_type_id, element_id)
    ...
    console.error('[api/user/solved-quest]', err);
    return Response.json({ error: 'Failed to mark as solved' }, { status: 500 });
  }
}
